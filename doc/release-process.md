...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in aprncoin, then:
...
### Build and sign aprncoin Core for Linux, Windows, and macOS:
...
Build output expected:
  ...
  4. macOS unsigned installer and dist tarball (`aprncoin-${VERSION}-osx-unsigned.dmg`, `aprncoin-${VERSION}-osx64.tar.gz`)
  5. Gitian signatures (in `gitian.sigs.ltc/${VERSION}-<linux|{win,osx}-unsigned>/(your Gitian key)/`)

### Verify other gitian builders signatures to your own. (Optional)

Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../aprncoin/contrib/gitian-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer aprncoin-osx-unsigned.tar.gz to macOS for signing
...
space *do not upload these to the aprncoin.org server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the aprncoin.org server.

```
- Update aprncoin.org version

- Announce the release:

  - aprncoin-dev and aprncoin-dev mailing list

  - blog.aprncoin.org blog post
