aprncoin Core version *0.15.1* is now available from:

  <https://download.aprncoin.org/aprncoin-0.15.1/>
...
A number of changes to the way aprncoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 saucers per kB (0.00001 APRN). This is to prepare for dropping the minimum transaction fee to 0.1 saucers per kB (0.0001 APRN) in the next release.
