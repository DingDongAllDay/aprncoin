...
- [Development guidelines](#development-guidelines)
    - [General aprncoin Core](#general-aprncoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
aprncoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/aprncoin-project/aprncoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_aprncoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_aprncoin --log_level=test_suite
$ valgrind -v --leak-check=full src/aprncoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_aprncoin.coverage/index.html`.
```

**Sanitizers**

aprncoin Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadRPCServer : Remote procedure call handler, listens on port 1952 for connections and services them.
...
file descriptors it will cause problems with aprncoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to aprncoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `aprncoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `aprncoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of aprncoin Core, the wallet was always
